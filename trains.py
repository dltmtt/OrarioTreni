...
class Train:
    ...
    def __str__(self):
        if hasattr(self, 'number_changes'):
            numbers = f'{self.train_number}{
                "/".join(c['new_train_number'] for c in self.number_changes)}'
        else:
...
def show_statistics():
    ...
    print(
        ...
        f'{S.DIM}Ultimo aggiornamento: {s['last_update'].strftime("%T")}{
            S.NORMAL}'
    )
...
def _process_train(t):
    ...
    tp = API.get_train_progress(
        t['origin_id'], t['number'], t['departure_date'])
